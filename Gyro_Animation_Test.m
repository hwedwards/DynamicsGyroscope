...
% Subplot 1: Alpha and AlphaDot vs Time
subplot(3, 1, 1);
hold on; 
plot(t, X(3,:), 'b', t, X(4,:), 'r');
xlabel('Time (s)');
ylabel('Alpha in Rad and Rad/s');
legend('Alpha', 'AlphaDot');
title('Alpha and AlphaDot vs Time');
hold off; 
% Subplot 2: Beta and BetaDot vs Time
subplot(3, 1, 2);
hold on; 
plot(t, X(1,:), 'b', t, X(2,:), 'r');
xlabel('Time (s)');
ylabel('Beta in Rad and Rad/s');
legend('Beta', 'BetaDot');
title('Beta and BetaDot vs Time');
hold off; 
% Subplot 3: Gamma and GammaDot vs Time
subplot(3, 1, 3);
hold on; 
plot(t, X(5,:), 'b', t, X(6,:), 'r');
xlabel('Time (s)');
ylabel('Gamma in Rad and Rad/s');
legend('Gamma', 'GammaDot');
title('Gamma and GammaDot vs Time');
hold off; 
% Adjust subplot spacing
spacing = 0.05;
position = get(gca, 'Position');
position(1) = position(1) - spacing;
position(3) = position(3) + spacing;
set(gca, 'Position', position);

% Enhance figure layout
sgtitle('Graph Subplot of Alpha, AlphaDot, Beta, BetaDot, Gamma, GammaDot vs Time');

%% ANIMATION
%% CREATE CYLINDERS
VIDEO = 1; 
%Gyroscope Axel - this is in frame 2
[yg,zg,xg] = cylinder(r3_num,100);
xg = xg*L_num - (L_num-d1_num); 


%Gyroscope CounterWeight - this is in frame 2
[yc, zc, xc] = cylinder(r2_num,100);
...
[x_stand, y_stand, z_stand] = cylinder(r3_num); 
z_stand = z_stand *L2_num; 
...
for i = 1:length(t)
    ...
    [xg_rotated, yg_rotated, zg_rotated] = rotation(xg, yg, zg, R02); 

    %Rotate the Counterweight
    [xc_rotated, yc_rotated, zc_rotated] = rotation(xc, yc, zc, R02); 

    %Rotate the Rotating Disk
    [xd_rotated, yd_rotated, zd_rotated] = rotation(xd, yd, zd, R02); 

    %Rotate the Stand
    [x_stand_rotated, y_stand_rotated, z_stand_rotated] = rotation(x_stand, y_stand, z_stand, R01); 

    % Adjust Heights

    zg_rotated = zg_rotated + L2_num; 
    zc_rotated = zc_rotated + L2_num; 
    zd_rotated = zd_rotated + L2_num; 
    %Surf colours the surface of a 3 dimensional plot
    surf(xg_rotated, yg_rotated, zg_rotated, yg_rotated, 'EdgeColor', 'none'); 
    surf(xc_rotated, yc_rotated, zc_rotated, yc_rotated, 'EdgeColor', 'none'); 
    surf(xd_rotated, yd_rotated, zd_rotated, yd_rotated, 'EdgeColor', 'none');
    surf(x_stand_rotated,y_stand_rotated, z_stand_rotated, y_stand_rotated); 
